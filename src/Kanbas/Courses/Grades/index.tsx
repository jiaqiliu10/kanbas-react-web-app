...
export default function Grades() {
    ...
    const enrolled_stu = enrollments.filter((enroll) => enroll.course === cid);

    const handleInputChange = (event: React.ChangeEvent<HTMLInputElement>, studentId: string) => {
        const newGrade = event.target.value;
        // Handle updating the grade here
        console.log(`New grade for student ${studentId}: ${newGrade}`);
    };
        ...
        <div id={"wd-grades-editor"}>
            ...
            <div className="table-responsive">
                <table className="table table-bordered">
                    ...
                    <tbody>
                        {enrolled_stu.map(enroll => {
                            const user = users.find(user => user._id === enroll.user);
                            const grade = grades.find(stu => stu.student === enroll.user);
                            return (
                                <tr key={enroll.user}>
                                    <td> {enroll.user} </td>
                                    <td className="text-danger">{user?.firstName}</td>
                                    <td className="text-danger">{user?.lastName}</td>
                                    <td>
                                        {grade && Number(grade.grade) === 90 ? (
                                            <input
                                                type="text"
                                                defaultValue={grade.grade}
                                                className="form-control short-input"
                                                onChange={(e) => handleInputChange(e, enroll.user)}
                                            />
                                        ) : (
                                            grade?.grade
                                        )}
